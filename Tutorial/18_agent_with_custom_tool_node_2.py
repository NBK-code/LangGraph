...
def agent(state: AgentState) -> AgentState:
    system_prompt = SystemMessage(content=("Be concise. If the user asks to uppercase/shout text, "
        "CALL the tool `shout` with the text. Otherwise answer briefly."))
...
print("\n--- Tool path ---")
inputs = {"messages": [("user", "Please shout: LangGraph is awesome!")]}
print_stream(app.stream(inputs, stream_mode="values"))

print("\n--- Direct answer path ---")
inputs = {"messages": [("user", "What is a custom tool node, in one line?")]}
print_stream(app.stream(inputs, stream_mode="values"))
